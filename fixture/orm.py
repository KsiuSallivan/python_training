...
class ORMFixture:
    ...
    # по группе ищем контакты, которые в нее входят (выбираем первую группу в списке)
    @db_session
    def get_contacts_in_group(self, group):
        orm_group = list(select(g for g in ORMFixture.ORMGroup if g.id == group.id))[0]
        return self.convert_contacts_to_model(orm_group.contacts)

    # по группе ищем контакты, которые в нее НЕ входят (выбираем первую группу в списке)
    @db_session
